...
export interface FluidMapProps<M extends Marker = Marker>
...
export function FluidMap<M extends Marker = Marker>({
      ...
      for (const i of activeSet) {
        if (mouse.active) {
          ...
          if (dist2 < R2 && dist2 > 0.0001) {
            ...
            // Clockwise tangential swirl
            vx[i] += (distY / dist) * force
            vy[i] += (-distX / dist) * force
            // Small radial push to prevent collapse
  ...
  return (
    ...
    >
      <defs>
        <filter id="fluid-map-marker-glow" x="-200%" y="-200%" width="500%" height="500%">
      ...
      {processedMarkers.map((marker, index) => {
        ...
        return (
          <g key={`${marker.x}-${marker.y}-${index}`} filter="url(#fluid-map-marker-glow)">
